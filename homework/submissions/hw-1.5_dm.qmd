...
2.  Load `laryngoscope.csv` from the `data` folder using `read_csv()`. If you get an error, make sure you have quotation marks around the file name, that this homework file is in your session 1 folder, and that `read_csv()` is looking in the `data` folder.
...
# your code here 
names(laryngoscope)
attempt1_time = laryngoscope$attempt1_time
signif(mean(attempt1_time, 2))
```
...
bmi_imperial = 703 * weight_lbs / height_in^2
mean(bmi_2) 
# mean is 39.61999 for second group 
bmi_1 = laryngoscope$BMI
mean(bmi_1)
# mean is 41.91722 for first group
# second group of patients has a lower mean BMI than laryngoscope patients
```
...
#fixed code below
correct_mean = mean(c(203.0, 150.1, 102.8, 241.0, 189.1, 148.3))
signif(correct_mean, 4)
```
