...
laryngoscope%>%
  ggplot()+
  geom_point(aes(x=age, y=ease, color=ease))+
labs(x="Age")+
labs(y="Ease")
```
...
polyps %>%
ggplot()+
  geom_density(aes(x=number12m, color=sex))+
  labs(x="12-month poly count")+
  labs(y="Density")+
  labs(title = "Male subjects had lower 12-month polyp counts")
...
```{r}
polyps %>% group_by(treatment,sex) %>% summarize(n(),median_baseline=median(baseline)) %>% 
  ggplot()+
  geom_bar(aes(x=median_baseline, y=treatment, fill=sex), stat='identity', position="dodge")
```
